...
mod platform {
    ...
    use windows_sys::Win32::{
        ...
        System::Threading::{
            CREATE_SUSPENDED, CreateProcessW, PROCESS_INFORMATION, ResumeThread,
    ...
    impl SuspendedChild {
        fn create(
            ...
            let process_handle =
                unsafe { OwnedHandle::from_raw_handle(process_information.hProcess) };
            // SAFETY: same invariant as above, for the primary thread handle.
            let primary_thread =
                unsafe { OwnedHandle::from_raw_handle(process_information.hThread) };

            Ok(Self {
                process: TargetProcess::from_created(
        ...
        fn resume(&mut self) -> Result<()> {
            ...
            let previous_suspend_count =
                unsafe { ResumeThread(self.primary_thread.as_raw_handle()) };
    ...
    mod tests {
        ...
        fn process_creation_does_not_inherit_handles() {
            ...
            let arguments = [
                ...
                OsString::from("--exact"),
                OsString::from(
                    "launch::platform::tests::inheritable_handle_is_unavailable_in_child",
                ),
            ];
            ...
            let succeeded = unsafe {
                GetExitCodeProcess(child.process().handle().as_raw_handle(), &mut exit_code)
            };
        ...
        fn inheritable_handle_is_unavailable_in_child() {
            ...
            let handle = fs::read_to_string("handle.txt")
                ...
                .parse::<usize>()
                .expect("handle probe was not a usize")
