use darpc_protocol::{
    ...
    MessageDirection, Ping, SequenceCounter, SequenceError, SessionError, VersionRange,
    elapsed_tick_ms, negotiate_version,
};
...
fn dll_and_controller_complete_the_same_handshake() {
    ...
    assert_eq!(dll.selected_version(), Some(1));
...
fn invalid_and_unsupported_versions_are_distinct() {
    ...
    assert_eq!(
        negotiate_version(VersionRange { min: 2, max: 3 }),
        Err(SessionError::UnsupportedVersionRange { min: 2, max: 3 })
    );
}
...
fn dll_rejects_an_acknowledgement_for_the_wrong_offer() {
    ...
    assert_eq!(
        ...
        Err(SessionError::InvalidSelectedVersion { selected: 2 })
    );
