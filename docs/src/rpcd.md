...
connection, it obtains a fresh snapshot from `darpc.dll` and then follows later
updates from an ordered stream boundary.

## Failure isolation

A daemon restart must not end a game session. Each `darpc.dll` continues tracking
local state while disconnected. When `darpcd.exe` returns, daemon-driven
